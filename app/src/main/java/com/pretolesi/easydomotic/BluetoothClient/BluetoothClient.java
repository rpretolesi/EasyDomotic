...
/*
        ...
        if(obj != null){
            if(obj[0] instanceof TcpIpClientStatus){
                if(m_vClientStatusListener != null) {
                    for (TcpIpClientStatusListener ticl : m_vClientStatusListener) {
                        ticl.onTcpIpClientStatusCallback((TcpIpClientStatus) obj[0]);
                    }
                }
            }
            if(obj[0] instanceof TcpIpClientWriteStatus){
                if(m_vClientWriteStatusListener != null) {
                    for (TcpIpClientWriteStatusListener ticwsl : m_vClientWriteStatusListener) {
                        ticwsl.onWriteValueStatusCallback((TcpIpClientWriteStatus) obj[0]);
                    }
                }
            }
            if(obj[0] instanceof TcpIpClientReadStatus){
                if(m_vClientReadValueStatusListener != null) {
                    for (TcpIpClientReadValueStatusListener ticrvsl : m_vClientReadValueStatusListener) {
